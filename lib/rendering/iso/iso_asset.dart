...
class IsoAssetLoader {
  ...
  /// Load a group of pre-sliced OBJ cells and generate iso sprites with
  /// pre-assigned grid cells.
  ///
  /// [cellSources] maps `(col, row)` — i.e. the `(x, y)` from the filename
  /// `{x}_{y}.obj` — to the raw OBJ source string. The cells are merged into
  /// a single [Geometry] whose faces carry their grid assignment, bypassing
  /// [GeometrySlicer] entirely.
  Future<IsoAsset> generateFromObjGroup(
    String id,
    Map<(int, int), String> cellSources, {
    Color color = Colors.white,
    double scale = 1.0,
  }) async {
    if (_cache.containsKey(id)) {
    ...
    // Parse each cell OBJ without centering so all cells share the same
    // tile-local coordinate frame.
    final allVertices = <Vector3>[];
    final allFaces = <List<int>>[];
    final faceGridCells = <(int, int)>[];

    for (final entry in cellSources.entries) {
      final (col, row) = entry.key;
      final source = entry.value;
      final cellGeometry = ObjParser.parse(
        id: '${id}_${col}_$row',
        name: '${id}[$col,$row]',
        source: source,
        center: false,
      ...
      final vertexOffset = allVertices.length;
      allVertices.addAll(cellGeometry.vertices);
      for (final face in cellGeometry.faces) {
        allFaces.add([for (final idx in face) idx + vertexOffset]);
        faceGridCells.add((row, col));
      }
    }

    // Center the combined geometry (same transform applied uniformly).
    // ObjParser already converted Rhino Z-up to engine Y-up, so vertices
    // are in the correct coordinate system (X,Z = ground, Y = height).
    _centerGeometryInPlace(allVertices);
    ...
    // Safety net: ensure consistent outward-facing winding after merge.
    final outward = ensureOutwardFacingGeometry(combined);

    final result =
        await _vectorGenerator.generateAllViewSpritesFromGeometryWithGrid(
      outward,
      color,
      faceGridCells: faceGridCells,
      scale: scale,
    );
  ...
  /// Centers geometry in-place: X/Z centered at origin, Y bottom at 0.
  static void _centerGeometryInPlace(List<Vector3> vertices) {
